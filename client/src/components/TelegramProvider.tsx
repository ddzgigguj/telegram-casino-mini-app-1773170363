...
export function TelegramProvider({ children }: TelegramProviderProps) {
  ...
  useEffect(() => {
    ...
    if (tg) {
      ...
      // Get user from Telegram - NO guest fallback allowed
      if (tg.initDataUnsafe?.user) {
        setTelegramUser(tg.initDataUnsafe.user);
      } else {
        // No valid Telegram user - don't create guest
        console.log("No initDataUnsafe.user, Telegram authentication required");
